...
| Stage | Skill | Role |
|---|---|---|
| Birth | `py-new` | Scaffold uv + ruff + ty + pytest + prek project, green verify loop from commit zero |
| Design | `grill-me` | One-question-at-a-time interview; updates CONTEXT.md/ADRs inline when the project keeps them |
| Build | `tdd` | Red-green-refactor, vertical slices; Python testing + faking rules |
| Broken | `diagnose` | Feedback-loop-first discipline for hard bugs; hands repro to `tdd`-style regression test |
...
- **`tdd` went 6 files -> 2** (`SKILL.md`, `tests.md`). TypeScript/jest examples rewritten in Python (pytest, monkeypatch, hand-written fakes, `Protocol`). `deep-modules.md` + `interface-design.md` folded into `_shared/LANGUAGE.md` references; `refactoring.md` folded into the Refactor step. Faking preference order added: real thing -> hand-written fake -> monkeypatch -> MagicMock. New: RED must be an assertion failure (not a collection error), bug-fix entry point, verify-loop exit.
- **`diagnose`** kept whole — strongest original. Added: scope note (ordinary bugs don't need it), `hypothesis` for fuzz loops, `tmp_path`/injected-clock determinism examples, throwaway harnesses follow the `spike_*.py` convention, full verify loop in Phase 6 checklist.
...
- **`py-new`** copied in unchanged (canonical AGENTS.md template lives in its `templates/`).
...
- Spikes and throwaway harnesses: `spike_*.py`, `THROWAWAY` header, never merged.
